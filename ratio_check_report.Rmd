...
# Annotation file and TMT regex patterns only needed for TMT / TMTpro
if (is_tmt) {
...
df_label <- df_psm %>%
  ...
  mutate(is_M            = grepl("15.9949", assigned_modifications),
         is_K_tmt        = grepl(tmt_regex, assigned_modifications),
         is_nterm_tmt    = grepl("N-term", assigned_modifications),
         has_M           = grepl("M", peptide),
         has_K           = grepl("K", peptide),
         potential_nterm = TRUE) %>%
  pivot_longer(!c(peptide, assigned_modifications),
               names_to = "mod", values_to = "status") %>%
  filter(status == TRUE) %>%
  group_by(mod) %>%
  count() %>%
...
if (exists("tmt2")) {
  df_tmt2 <- df_psm %>%
    select(peptide, 
           assigned_modifications) %>%
    mutate(has_K     = grepl("K", peptide),
           is_K_tmt2 = grepl(tmt2, assigned_modifications)) %>%
    pivot_longer(!c(peptide, assigned_modifications),
                 names_to = "mod",
                 values_to = "status") %>%
    filter(status == TRUE) %>%
